...
| R10b | An additional derived type is registered | Compatible | Compatible | Compatible | Earlier documents keep resolving because their discriminator values are still registered. This rule applies when the earlier contract already has polymorphic dispatch. Adding polymorphic dispatch to an object with earlier writable extension data is a separate fail-closed interaction: the earlier arbitrary key space can contain the later discriminator property name with an unrecognized value, so the shipping ReaderBackward comparison reports `Unsupported` under `R10c.polymorphism.extension-data-discriminator-collision`. Proof: `R10.polymorphism.derived-type-added`, `.forward`, `.full`, and `ComparisonConservatismTests.ExtensionDataUnknownDiscriminatorFailsClosedWhenPolymorphismIsAdded`. |
...
The canonical document describes the effective contract in a stable form: format version `3` for the current
