...
| R10e | Polymorphic dispatch is added while the declared base remains concrete | Compatible | Compatible | Compatible | The measured base materializer can still read an earlier document without a discriminator, and the concrete-base control round-trips without loss. This result does not override the R10c extension-data interaction. Proof: `R10e.polymorphism.dispatch-added-concrete`, `.forward`, `.full`, and `ComparisonConservatismTests.ConcreteToAbstractPolymorphicTransitionRequiresADiscriminator`. |
| R10f | The later object contract is abstract or an interface without registered polymorphic derived-type metadata | Unsupported | Unsupported | Unsupported | The framework reader cannot construct the declared type from an earlier object document. Reader materialization is recorded even when `JsonPolymorphismOptions` is absent, and the contract fails closed as `unsupported.object-materialization-unproven`. Proof: `ComparisonConservatismTests.PlainAbstractAndInterfaceReadersFailClosed` and the isolated package-consumer regression, for reflection and source-generated metadata, baseline write/read, `Compare`, and `AssertCompatible`. |
...
| `unsupported.object-materialization-unproven` | Unsupported | An abstract or interface object contract has no registered polymorphic derived-type metadata the framework reader can materialize. |
...
`D06.comparison-fact-interactions.coverage` gate compares all 78 rows with the code catalogue. Adding a fact
family or assigning a new compared fact kind to no family leaves pairs unclassified and fails the gate.
...
| `member-identity + nullability` | `Cannot interact` | `the facts are checked independently after member selection, and the worst verdict is retained; neither fact changes the JSON key or token evaluated by the other` | `Not applicable` |
| `member-identity + member-materialization` | `Cannot interact` | `the facts are checked independently after member selection, and the worst verdict is retained; neither fact changes the JSON key or token evaluated by the other` | `Not applicable` |
...
| `member-identity + container-materialization` | `Cannot interact` | `container materialization is classified on the selected member shape and cannot change which serialized name selected that member` | `Not applicable` |
...
| `member-identity + polymorphism` | `Cannot interact` | `member names and discriminator values occupy separate canonical namespaces; derived object members are compared through the reference graph` | `Not applicable` |
...
| `member-inclusion + nullability` | `Cannot interact` | `the presence rule is decided before a value token is interpreted, and the value-domain rule cannot make a missing member present` | `Not applicable` |
...
| `member-inclusion + token-domain` | `Cannot interact` | `the presence rule is decided before a value token is interpreted, and the value-domain rule cannot make a missing member present` | `Not applicable` |
| `member-inclusion + container-shape` | `Cannot interact` | `the presence rule is decided before a value token is interpreted, and the value-domain rule cannot make a missing member present` | `Not applicable` |
| `member-inclusion + container-materialization` | `Cannot interact` | `the presence rule is decided before a value token is interpreted, and the value-domain rule cannot make a missing member present` | `Not applicable` |
| `member-inclusion + enum-domain` | `Cannot interact` | `the presence rule is decided before a value token is interpreted, and the value-domain rule cannot make a missing member present` | `Not applicable` |
...
| `member-inclusion + polymorphism` | `Cannot interact` | `member inclusion does not register or remove discriminator values; included members of derived objects are reached through the reference graph` | `Not applicable` |
...
| `requiredness + token-domain` | `Cannot interact` | `the presence rule is decided before a value token is interpreted, and the value-domain rule cannot make a missing member present` | `Not applicable` |
| `requiredness + container-shape` | `Cannot interact` | `the presence rule is decided before a value token is interpreted, and the value-domain rule cannot make a missing member present` | `Not applicable` |
| `requiredness + container-materialization` | `Cannot interact` | `the presence rule is decided before a value token is interpreted, and the value-domain rule cannot make a missing member present` | `Not applicable` |
| `requiredness + enum-domain` | `Cannot interact` | `the presence rule is decided before a value token is interpreted, and the value-domain rule cannot make a missing member present` | `Not applicable` |
...
| `requiredness + polymorphism` | `Cannot interact` | `required object members and registered discriminator values are separate presence checks; derived member requirements are reached recursively` | `Not applicable` |
| `nullability + member-materialization` | `Cannot interact` | `the facts are checked independently after member selection, and the worst verdict is retained; neither fact changes the JSON key or token evaluated by the other` | `Not applicable` |
...
| `nullability + container-shape` | `Cannot interact` | `null acceptance belongs to the value slot outside its non-null container shape, so both checks run without either relaxing the other` | `Not applicable` |
| `nullability + container-materialization` | `Cannot interact` | `null rejection occurs before a container materializer is invoked and cannot be repaired by that materializer` | `Not applicable` |
| `nullability + enum-domain` | `Cannot interact` | `the null token lies outside both named and integer enum domains and is checked independently at the value slot` | `Not applicable` |
...
| `nullability + polymorphism` | `Cannot interact` | `a null token is handled before discriminator dispatch, while non-null derived contracts are compared recursively` | `Not applicable` |
| `member-materialization + token-domain` | `Cannot interact` | `the facts are checked independently after member selection, and the worst verdict is retained; neither fact changes the JSON key or token evaluated by the other` | `Not applicable` |
| `member-materialization + container-shape` | `Cannot interact` | `member writability controls assignment of the selected value; it does not alter that value's JSON shape` | `Not applicable` |
...
| `member-materialization + enum-domain` | `Cannot interact` | `the facts are checked independently after member selection, and the worst verdict is retained; neither fact changes the JSON key or token evaluated by the other` | `Not applicable` |
...
| `member-materialization + polymorphism` | `Cannot interact` | `member assignment occurs after derived-type dispatch; materialization within a derived object is checked through the reference graph` | `Not applicable` |
...
| `token-domain + container-materialization` | `Cannot interact` | `the families apply to mutually exclusive canonical node kinds, so one node cannot carry both facts; nested nodes are covered through the reference-graph pairs` | `Not applicable` |
...
| `token-domain + polymorphism` | `Cannot interact` | `discriminator dispatch selects a derived node before that node's scalar or member token domains are compared recursively` | `Not applicable` |
...
| `container-shape + enum-domain` | `Cannot interact` | `the families apply to mutually exclusive canonical node kinds, so one node cannot carry both facts; nested nodes are covered through the reference-graph pairs` | `Not applicable` |
...
| `container-materialization + enum-domain` | `Cannot interact` | `the families apply to mutually exclusive canonical node kinds, so one node cannot carry both facts; nested nodes are covered through the reference-graph pairs` | `Not applicable` |
...
| `enum-domain + polymorphism` | `Cannot interact` | `enum value tokens and polymorphic discriminator tokens are selected at different nodes; derived enum members are compared recursively` | `Not applicable` |
...
The canonical document describes the effective contract in a stable form: format version `4` for the current
...
nodes also record whether their concrete materializer is constructible under the measured rule. Scalar
