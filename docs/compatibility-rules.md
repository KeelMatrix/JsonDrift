...
| R10g | A polymorphic discriminator property name collides with an effective ordinary member name | Unsupported | Unsupported | Unsupported | The discriminator is a JSON property on the wire, not a separate namespace. The reader can consume an ordinary member value as dispatch metadata and reject it as an unknown discriminator, so the contract fails closed as `unsupported.polymorphism-discriminator-member-collision`. Proof: `ComparisonConservatismTests.DiscriminatorMemberNameCollisionsFailClosed` and the isolated package-consumer regression, including the non-colliding R10e positive control. |
...
| `unsupported.polymorphism-discriminator-member-collision` | Unsupported | A polymorphic discriminator JSON property name collides with an effective ordinary serialized member name. |
...
| `member-identity + polymorphism` | `Interacts` | `a discriminator is a JSON property name, so a colliding effective ordinary member can be reinterpreted as dispatch metadata and must fail closed` | `test:DiscriminatorMemberNameCollisionsFailClosed` |
